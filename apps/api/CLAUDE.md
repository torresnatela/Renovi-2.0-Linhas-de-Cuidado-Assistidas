...
| Conclusão do onboarding (convite) | `controllers/onboarding_controller.go` → `models/onboarding.go` — rotas PÚBLICAS `/onboarding/{token}` (token na URL = credencial); reusa `AccountStore.Register` (ADR-044) |
