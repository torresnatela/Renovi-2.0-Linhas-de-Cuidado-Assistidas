...
- [~] Ativação via Gestão — **ingestão (push, ADR-043, 0016) + conclusão do onboarding (ADR-044, 0017) FEITAS** (backend + frontend): recebe contratos, cunha o convite, e o link `/onboarding/{token}` conclui o cadastro (reusa a saga do `/auth/register`, confere o CPF por HMAC) e fecha o vínculo (`vinculado` + `accepted_at`); a recusa marca `recusado`. **Falta:** aceite logado com consentimento para quem já tem conta (`cpf_match`, casos 4–5); e-mail SMTP real (hoje `invite_url`/WhatsApp); matrícula automática em linha de cuidado a partir do contrato aceito. Mudou de "Adapter Gestão (leitura/pull)" para PUSH — supera o pull do ADR-009.
