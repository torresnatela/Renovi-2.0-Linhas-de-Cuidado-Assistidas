...
- [~] Ativação via Gestão — **fundação de ingestão (push) FEITA** (ADR-043, migration 0016): recebe contratos, persiste vínculos idempotente, cunha o `onboarding_token` e emite convite (stub). **Falta:** conclusão do onboarding (token → conta + fecha vínculo), `cpf_match` com consentimento (casos 4–5), e-mail SMTP real, frontend do convite. Mudou de "Adapter Gestão (leitura/pull)" para PUSH — supera o pull do ADR-009.
