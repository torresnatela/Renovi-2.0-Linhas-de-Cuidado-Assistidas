...
_Última atualização: 2026-07-22 — **Conclusão do onboarding pelo convite concluída (backend + frontend)** (ADR-044): o `invite_url` agora leva a um fluxo completo. Rotas PÚBLICAS `/onboarding/{token}` (`GET` pré-preenchimento · `POST .../complete` · `POST .../decline`) — o token na URL é a credencial. `complete` reusa a saga `AccountStore.Register` (valida + DAV + enrollment universal) e fecha o vínculo numa TX curta (`gestao_employee_link`→`vinculado`/`convite`/`linked_at` + `accepted_at` nos contratos vivos + token consumido + evento `onboarding_concluido`); confere o CPF por HMAC (`CPF_MISMATCH` senão); quem já tem conta → 409 `ALREADY_HAS_ACCOUNT` (aceite logado é fatia futura). `decline` marca `status='recusado'` + revoga o token + `onboarding_recusado`. Migration `0017` (status `recusado` + 2 event types). Front: rota pública `/onboarding/:token` reusando o wizard do `/cadastro` (semeado do convite; CPF digitado) + passo de confirmação da empresa (SIM fácil, NÃO com dupla confirmação). Testes verdes: unit + integração (testcontainers) + controller + router + vitest (285). Supera `ARQUITETURA §3`/`ADR-009` no mecanismo de ativação (mas seguimos nunca escrevendo na Gestão).
Antes: 2026-07-22 — **Ingestão da Gestão — fundação (push) concluída no backend** (ADR-043): a Gestão passa a CHAMAR nossa API (`POST /integration/gestao/contracts` idempotente + `.../employees/{cpf_hmac}/resend-invite`), atrás de `X-Integration-Token`. Migration `0016` (5 tabelas: `gestao_company_link`, `gestao_employee_link`, `gestao_contract`, `onboarding_token`, `gestao_ingestion_event` + `patient_identity.cpf_hmac`). Chave da pessoa é `cpf_hmac` (HMAC-SHA256, pepper compartilhado; `cpf.HMAC` + `cmd/backfill-cpf-hmac`); CPF nunca trafega. Convite via `Notifier` stub (`adapters/notify`), com `invite_url` na resposta. `cpf_match` é DETECTADO mas NÃO auto-vincula (consentimento fica p/ fatia futura). Testes unitários verdes; integração (testcontainers) escrita, roda com `make test-integration`. Supera `ARQUITETURA §3`/`ADR-009` no mecanismo de ativação (mas seguimos nunca escrevendo na Gestão).
...
- [~] Ativação via Gestão — **ingestão (push, ADR-043, 0016) + conclusão do onboarding (ADR-044, 0017) FEITAS** (backend + frontend): recebe contratos, cunha o convite, e o link `/onboarding/{token}` conclui o cadastro (reusa a saga do `/auth/register`, confere o CPF por HMAC) e fecha o vínculo (`vinculado` + `accepted_at`); a recusa marca `recusado`. Mudou de "Adapter Gestão (leitura/pull)" para PUSH — supera o pull do ADR-009.
  - **O que FALTA para a feature ficar concluída** (os 5 casos de uso da SPEC — hoje cobrimos os casos 1–3):
    - [ ] **Casos 4/5 — aceite por quem JÁ tem conta, com consentimento.** Hoje `complete` responde 409 `ALREADY_HAS_ACCOUNT` e a ingestão só DETECTA/registra `cpf_match_pendente`. Falta: **(caso 4)** notificar o paciente existente e deixá-lo **aceitar logado** o vínculo → fecha com `link_method='cpf_match'`; **(caso 5)** durante o autocadastro (`/cadastro`), se houver `gestao_employee_link` pendente para o CPF, perguntar e fechar no fim do cadastro. Precisa de rota(s) tipo `GET/POST /me/pending-links` (listar + aceitar/recusar) e UI in-app. **Segurança:** nunca abrir sessão/fechar vínculo sem autenticar o titular.
    - [ ] **E-mail SMTP real (caso 2 "de verdade").** Trocar o `LogNotifier` stub por um adapter real (ex.: Resend/SES) implementando a MESMA porta `Notifier`/`InviteMessage` em `adapters/notify` (troca limpa, sem mexer em model/rotas). Hoje o convite chega por `invite_url`/WhatsApp. Precisa de credenciais na config + template + disciplina LGPD (nunca logar o link/token).
    - [ ] **Matrícula automática a partir do contrato aceito** (o que de fato libera as consultas/questionários). O gancho já existe (`enrollment.gestao_contract_id`, `enrollment_period.source='gestao'`); `accepted_at` é setado na conclusão, mas ainda **não vira matrícula**. Regra do modelo: habilitar o benefício quando `status IN ('ativo','afastado') AND accepted_at IS NOT NULL` → matricular na(s) linha(s) de cuidado da empresa.
    - [ ] **Pontas soltas (robustez):** desligamento (`status='desligado'`) **revogar** a matrícula/benefício; **re-convite após recusa** (`recusado` hoje é "sticky", sem caminho de volta); UX de **multi-empresa** (paciente com vários vínculos); **observabilidade/admin** dos vínculos (hoje só o log append-only `gestao_ingestion_event`).
